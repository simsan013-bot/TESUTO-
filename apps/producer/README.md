...
チェッカー⑥〜⑦の判定ロジック自体は実装済みだが、`編集`/`サムネ`は
App自体が未実装のためexecutorが存在せず、判定にも到達しない。
...
`画像`は従来手作業だった「登場人物設計→キャラ別プロンプト生成→参照画像生成→
ラベル登録→シーンプロンプトでの参照」までを`runImageStage_`が1工程内で連結する
形で接続済み（チェッカー⑤、詳細は次節）。
未解決の`編集`/`サムネ`は別途解消が必要な既知の課題で、解消され次第、追加実装なしで
チェッカーが自動的に発火するようにしてある。

### `画像`工程の自動化内容

従来は (1) シナリオ設計の【登場人物設計】からキャラクタープロンプトを作成、
(2) キャラクター参照画像を生成、(3) 画像生成Appの参照画像シートにURL＋
キャラクター名を手動登録、(4) シーン画像プロンプト生成時にその参照キャラを
`[[名前]]`で指定、という4ステップを人間が手作業で行っていた。
`runImageStage_`（`apps/producer/ExternalApps.gs`）がこれを1工程内で連結する：

1. アナリストFB修正後Docの`design`をPRP生成Appの`character`モードに渡し、
   キャラクター別プロンプトを生成
2. 各キャラの正面プロンプトだけを画像生成Appの`generate`モードで1枚ずつ
   画像化し、生成URLを画像生成Appに新設した`registerRef`モードで
   ワーク単位のラベル（`No{作品No}_名前`）として登録
   （`apps/image-generator/README.md`「producer連携」参照）
3. 圧縮工程が出力したスプシのタブ1「台本」を読み戻した本文をPRP生成Appの
   `scene`モードに渡し、シーンプロンプトをバッチ生成（`apps/prp-generator/README.md`
   「producer連携」参照）
4. シーンプロンプト中の`[[名前]]`参照をワーク単位ラベル`[[No{作品No}_名前]]`に
   文字列置換してから画像生成Appの`generate`モードに渡す

画像生成App自体は全ワーク共通の参照画像シートを1枚しか持たない
（コンテナバインドのため）ため、ラベルをワーク単位にすることで作品間の
衝突を避けている。各Appのプロンプト・判定ロジックは無改修。
...
`音声`/`猫感想`/`画像`はこのDoc方式ではなく、それぞれ別の経路で入力を得ている
（`音声`=`キャラ別台本`が同スプシに書いた「キャラ一覧」/「生成Doc一覧」タブ、
`猫感想`=`アナリストFB`Docのsteps結合文、`画像`=`アナリストFB`Docのdesignと
`圧縮`工程のスプシのタブ1「台本」読み戻し。前節「`画像`工程の自動化内容」参照）。
